...
** Customization File
Our customisations are stored in =\~/emacs/custom.el=.
...
Otherwise, if the customizations file doesn't exist locally, copy it from git.
  ...
  #+begin_src elisp :tangle yes
    (if (not (file-exists-p "~/emacs/custom.el"))
      (progn
        (if (not (file-directory-p "~/emacs"))
    	(make-directory "~/emacs"))
        (copy-file "~/.emacs.d/custom.el" "~/emacs/custom.el")))
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; Old version of the code. more complicated. ignore.
    ;;  but i'm keeping it around in case it's needed :)
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
** LSP Mode Setup
Sets up LSP from scratch, in case variables are missing.
#+begin_src elisp
  (lsp)
#+end_src
...
** C Mode
We'll add a new C style, and set that as the default:
#+begin_src elisp
  (c-add-style "default" (list "gnu" '(c-basic-offset . 4)))
#+end_src
* Hooks
** Org Mode
...
#+begin_src elisp
  ...
  (defmacro lsp-attach-hook (&rest args)
    `(progn ,@(mapcar #'(lambda (a) `(add-hook ,a 'lsp)) args) t))

  (lsp-attach-hook 'java-mode-hook
  		 'c++-mode-hook
  		 'c-mode-hook
  		   )
#+end_src
...
#+end_src
** Java Mode
Disable =indent-tabs-mode= specifically for =java-mode= so that it doesn't keep awkwardly indenting, when =format-all-mode= is doing all the work for me.
#+begin_src elisp
  (defun java-mode-hook-enable ()
    (indent-tabs-mode 0)
    (format-all-mode 1)
    (setq format-all-formatters '(("Java" astyle)))
    )

  (add-hook 'java-mode-hook 'java-mode-hook-enable)
#+end_src
...
** Define Keys
This macro is for defining multiple keys for one map at once.
It expands a list of (key,val) pairs into separate =(define-key)= statements.
...
** Show Keybinds
#+begin_src elisp
  (defun show-keybinds ()(interactive)
         (momentary-string-display (with-temp-buffer (setq org-mode t)(insert-file-contents "~/org/important-keybinds.org")  (buffer-string)) (point)))
#+end_src
* Package Config
** Ace Window
*** Disable ignored buffers
#+begin_src elisp
  (setq aw-ignore-on nil)
#+end_src
** Corfu
** GDScript
Ignore LSP mode errors!
#+begin_src elisp
...
This is for org-roam keybinds - specifically, so that I can easily add links inline while typing. It uses =C-c C-r= as the prefix.
...
#+begin_src elisp
  ...
  (define-key org-mode-map (kbd "C-c C-r") org-roam-map)
#+end_src

*** Treemacs Mode Map
Set up TAB and RET on =treemacs-mode=.
This is usually blocked by =evil=, so we simply redefine it using =evil='s keybinding system.
#+begin_src elisp
...
This hasn't seemed to do anything. Let's try defining it in a different way:
#+begin_src elisp
...
**** Extra keybinds for Rustic Mode Map
#+begin_src elisp
  ;(define-key rustic-mode-map (kbd "C-c C-c C-d") 'rustic-cargo-build-doc)
#+end_src

*** Corfu Mode Map
This lets us set specific keybinds for when dealing with Corfu autocomplete.
...
#+begin_src elisp
  ...
  (keymap-global-set "C-x <up>" 'electric-buffer-list)
...
***** File Maps
This map is for easily navigating between files.
#+begin_src elisp
  (defvar file-map (define-keymap
#+end_src

Go to recent files with =SPC f r=.
#+begin_src elisp
  "r" #'recentf
#+end_src
